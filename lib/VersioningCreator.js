...
function VersioningCreator(modelName){
    var finalised = false;
    var versioning = {
        'versions': {}
    };
    ...
    this.addVersion = function(versionNumber) {
        ...
        try {
            ...
            throw new Error('Expected teh version number to be an integer');
        }
    ...
    versioning.update = function(oldState) {
        ...
        Object.keys(versioning.versions).sort().forEach( (versionNumber) => {
            if ( error != null 
                || ! versioning.versions.hasOwnProperty(versionNumber) ) {
                    return;
                }
            if ( versionNumber <= newState.version ) {
                return;
            }
            newState.version = Number.parseInt(versionNumber);
